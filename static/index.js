function validateForm(x, y, r) {
    ...
    const sanitizedY = y.replace(',', '.')
    const parsedY = parseFloat(sanitizedY)

    if (isNaN(parsedY) || parsedY < -3 || parsedY > 3) {
...
document.querySelectorAll('.x-btn').forEach(button => {
    button.addEventListener('click', function() {
        selectedX = button.getAttribute('data-value')
...
function addResultToTable(data, currentTime) {
    ...
    row.innerHTML = `
            ...
            <td>${data.y}</td>
            <td>${data.r}</td>
            <td>${data.status ? 'Попадание' : 'Промах'}</td>
            <td>${data.time}</td>
            <td>${currentTime}</td>
        `
